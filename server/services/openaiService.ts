...
export async function splitPost(
  ...
  const strategiesDescription = strategiesArray.map((strategy) => getStrategyDescription(strategy)).join('\n\n')
    ...
    try {
      ...
      for (let i = 0; i < result.posts.length; i++) {
        if (typeof result.posts[i] !== 'string') {
          throw new Error(
            `Post at index ${i} is not a string. Type: ${typeof result.posts[i]}, Value: ${JSON.stringify(result.posts[i])}`
          )
        }
...
export async function generateSplittingOptions(
  ...
  const needsSplitting = platforms.some((platform) => content.length > platformLimits[platform])
 ...
 */
export async function optimizePost(content: string, platform: string, characterLimit: number): Promise<string> {
  try {
    const response = await openai.chat.completions.create({
      model: 'gpt-4o', // the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
      messages: [
