...
export default class extends Controller {
  ...
  updatePreview(tabId = null) {
    ...
    if (!tabId) {
      const activeTab = this.previewTabTargets.find(tab => 
  ...
  optimizeForPlatform(platformId) {
    ...
    if (stats && stats.percent > 70 && stats.percent < 100) {
      ...
      optimizeButton.classList.add('mt-4', 'px-3', 'py-1', 'bg-surface1', 'hover:bg-surface2', 
                                  'text-sm', 'text-primary', 'rounded-full', 'flex', 'items-center')
