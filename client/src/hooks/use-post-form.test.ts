...
describe('usePostForm hook', () => {
  ...
  it('should toggle platform selection', () => {
    ...
    const mastodonPlatform = result.current.formState.selectedPlatforms.find((p) => p.id === 'mastodon')
    ...
    if (mastodonPlatform) {
      ...
      const updatedMastodonPlatform = result.current.formState.selectedPlatforms.find((p) => p.id === 'mastodon')
