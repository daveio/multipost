...
export const createTheme = (flavor: CatppuccinTheme): Record<string, string> => {
  ...
  themeVars['--primary-foreground'] =
    flavor === 'latte' ? hslToVar(flavorData.colors.crust.hsl) : hslToVar(flavorData.colors.surface0.hsl)

  // Secondary colors
  themeVars['--secondary'] = hslToVar(flavorData.colors.mauve.hsl)
  themeVars['--secondary-foreground'] =
    flavor === 'latte' ? hslToVar(flavorData.colors.crust.hsl) : hslToVar(flavorData.colors.surface0.hsl)

  // Accent colors
  themeVars['--accent'] = hslToVar(flavorData.colors.pink.hsl)
  themeVars['--accent-foreground'] =
    flavor === 'latte' ? hslToVar(flavorData.colors.crust.hsl) : hslToVar(flavorData.colors.surface0.hsl)
  ...
  themeVars['--destructive-foreground'] =
    flavor === 'latte' ? hslToVar(flavorData.colors.crust.hsl) : hslToVar(flavorData.colors.surface0.hsl)
