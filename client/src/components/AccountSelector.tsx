...
export function AccountSelector({ accounts, selectedAccounts, onToggleAccount }: AccountSelectorProps) {
                  ...
                  {groupedAccounts[platformId].map((account) => (
                    <div key={account.id} className="flex items-center gap-2 p-2 bg-muted rounded-md">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {platformId === 'mastodon' && account.instanceUrl ? account.instanceUrl : 'Main Account'}
                      </span>
                    </div>
                  ))}
                </div>
