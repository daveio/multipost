...
import { mockThreadPosts, renderWithProviders } from '../../../test/test-utils'
...
describe('ThreadPostsManager component', () => {
  ...
  it('should render thread posts navigation', () => {
    ...
    for (const label of postLabels) {
      ...
      const matchingButton = allButtons.find((btn) => btn.textContent?.includes(label))
    ...
    const firstPostButton = allButtons.find((btn) => btn.textContent?.includes('Post 1'))
  ...
  it('should call onSwitchPost when another post is clicked', () => {
    ...
    const secondPostButton = allButtons.find((btn) => btn.textContent?.includes('Post 2'))
  ...
  it('should call onAddPost when add post button is clicked', () => {
    ...
    const allButtons = screen.getAllByRole('button')
    const addPostButton = allButtons.find(
      (btn) => btn.textContent?.includes('Add Post') && btn.querySelector('svg') // Has an icon
    )
  ...
  it('should call onRemovePost when remove button is clicked', () => {
    ...
    const removeButton = allButtons.find(
      (btn) => btn.textContent?.includes('Remove This Post') && btn.querySelector('svg.lucide-trash2')
    )
  ...
  it('should call onExit when exit thread mode button is clicked', () => {
    ...
    const exitButton = allButtons.find(
      (btn) =>
  ...
  it('should prevent default on button clicks to avoid form submission', () => {
    ...
    const postButtons = screen.getAllByRole('button').filter((btn) => btn.textContent?.includes('Post'))

    const addButton = screen.getAllByRole('button').find((btn) => btn.textContent?.includes('Add Post'))
